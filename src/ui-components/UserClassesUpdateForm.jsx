...
export default function UserClassesUpdateForm(props) {
  const {
    id: idProp,
    userClasses: userClassesModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userClassesRecord
      ? { ...initialValues, ...userClassesRecord }
      : initialValues;
    setUserClasses(cleanValues.UserClasses);
    setUserID(cleanValues.UserID);
    setClassID(cleanValues.ClassID);
    setRegistrationDate(cleanValues.RegistrationDate);
    setErrors({});
  };
  const [userClassesRecord, setUserClassesRecord] =
    React.useState(userClassesModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(UserClasses0, idProp)
        : userClassesModelProp;
      setUserClassesRecord(record);
    };
    queryData();
  }, [idProp, userClassesModelProp]);
  React.useEffect(resetStateValues, [userClassesRecord]);
  const validations = {
    UserClasses: [],
    UserID: [],
    ClassID: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            UserClasses0.copyOf(userClassesRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "UserClassesUpdateForm")}
      {...rest}
    >
      <TextField
        label="User classes"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userClassesModelProp)}
